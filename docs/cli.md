...
| `--detach` | fork the run into the background and return almost immediately: prints the bare run id on stdout and exits 0 once the child's `run.json` exists (within 8s — past that, exit 2 pointing at `<run dir>/detach-launch.log`). **POSIX only** — on a platform without `setsid` this is a usage error (exit 2) naming the platform. The exit code is the *launch* succeeding, not the workflow's eventual outcome; follow up with `rayspec logs <id> --follow`, list with `rayspec runs`, stop with `rayspec cancel <id>`. With `--no-worktree` (or `defaults.isolation: none`) a workdir lock already held by another run is caught immediately, before anything is backgrounded (exit 2, naming the holder); a worktree's lock is caught by the detached process itself, same as a foreground run. Because the detached process has no controlling terminal, an `approve:` gate it reaches pauses the run (exit 3 in the detached process) exactly as `--no-interactive` would — there is nothing to answer it with, so script an answer ahead of time (`--yes`, `--approve-class`, a policy) or plan to `rayspec approve`/`resume` it later |
...
Cancelling a live run is **cooperative, not a kill**: instead of signalling the process, `rayspec cancel` writes a small `cancel.json` marker (reason, actor, timestamp) next to `run.json` in the run directory (asks first unless `--yes` or `--json`; without a terminal to answer on it refuses with exit 2 and the `--yes` hint). The run's own process notices the marker at its next step boundary, lets any step already in flight finish, still runs `join: always` cleanup steps, and then finalizes itself — exit 4, same as before. Nothing outside the run's own process ever touches its steps or its workdir; there is no longer a window where a SIGINT lands mid-write. This needs no terminal on the run's side and works identically whether the run is in the foreground or was started with `rayspec run --detach` — watch it stop with `rayspec logs <id> --follow`. A run that is slow to reach a step boundary (deep inside one long provider call, for instance) keeps running until it does; cancel does not preempt it.
