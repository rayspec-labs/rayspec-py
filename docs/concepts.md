...
([constitution.md](constitution.md) is the tie-breaker for every "can we add a field" question).
...
| **run** | One execution of a workflow. It has an id (`YYYYMMDD-HHMMSS-xxxx`), fixed inputs, a working directory and a run directory under `~/.rayspec/projects/<slug>/runs/<run-id>/` that holds every checkpoint ([runs-and-resume.md](runs-and-resume.md)). |
...
[runs-and-resume.md](runs-and-resume.md).
...
> the secret and exposes one narrow tool ([examples/secret_via_tool](../examples/secret_via_tool/)).
...
runs against another project. See [isolation.md](isolation.md).
