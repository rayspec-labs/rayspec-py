...
| A sink that POSTs to ntfy/Slack/a webhook from the rayspec process | Rejected — sinks are **`exec:` only** | The engine may spawn a command; it may never open a socket. rayspec's claim that "the only network activity is what agents do through their SDKs, what workflow shell steps run themselves, and the one opt-in `git push` below" is a guarantee, not a description. An `exec:` sink keeps it literally true, keeps timeouts/retries/proxies/TLS/outbound-secret handling out of the engine, and needs no dependency. `examples/notify_webhook` already delivers a webhook from a `shell:` step — that is the idiom. |
| `RAYSPEC_PUSH_BRANCH`: publish the run's branch when it pauses or ends | Accepted, narrowly | The first network operation rayspec *initiates* rather than an agent or a `shell:` step, so it is written into the guarantee above rather than quietly excepted from it. It stays inside the rule that matters — the engine spawns `git`, it opens no socket of its own — and it is fenced in: off unless the variable is set, isolated runs only, never `--force`, never a dry run, and outcome-independent (any failure is a warning; the status and the exit code are what they would have been). Anything beyond publishing a branch (opening a PR, calling an API) is a `shell:` step. |
| `rayspec run --detach`: run in the background | Accepted, narrowly | It spawns a process and opens no socket — the same rule the engine already lives by. The launcher `subprocess.Popen`s one `setsid` child (an ordinary `rayspec run`) and hands back; there is no `serve`, no daemon, no queue, no supervisor, no IPC beyond a file handshake the child writes and the launcher reads. Fenced in: POSIX only, refused with `--resume`/`--stubs-init`, the child is `--no-interactive` (a gate pauses, it never waits on a socket for an answer), and everything after launch is the same file-based `runs`/`logs`/`cancel` a foreground run uses. |
