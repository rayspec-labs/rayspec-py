...
def test_json_mode_stream_shapes_match_cli_md(tmp_path: Path) -> None:
    ...
    for event in events:
        if event["type"] != "stream":
            ...
            # because the serialiser drops it whole when the microsecond component is zero —
            # a timestamp on the exact second renders ``…T03:04:05Z`` — a shape every clock
            # value must be allowed to take, and the same optional-fraction form the golden
            # capture normaliser accepts.
            ts = event["ts"]
