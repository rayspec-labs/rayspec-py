...
def test_resume_notes_a_project_override_it_does_not_load(
    cli: CliRunner, home: Path, project: Path, monkeypatch
) -> None:
    """B11 (F/eject): a run that recorded a bundled workflow, resumed after the project ejected a
    copy of the same name, is told plainly that the resume reloads the recorded (bundled) file."""
    import json as _json

    # start a run of a BUNDLED workflow (fix_issue) so workflow_path is <bundled>/…
    stubs = project / "s.yaml"
    stubs.write_text(
        "steps:\n  a: { fail: { kind: api, message: no, transient: false } }\n", encoding="utf-8"
    )
    started = cli.invoke(
        app,
        [
    ...
    # the run fails (stub), leaving it resumable; grab the id
    run_id = _json.loads(started.stdout.strip().splitlines()[-1])["run_id"]
    rec = _record(home, run_id)
    assert rec["workflow_path"].startswith("<bundled>/"), rec["workflow_path"]

    # now EJECT: a project copy of fix_issue appears
    (project / ".rayspec" / "workflows" / "fix_issue.yaml").write_text(
        BUNDLED_LIKE, encoding="utf-8"
    )
    resumed = cli.invoke(app, ["resume", run_id, "--root", str(project), "--force"])
    assert "recorded the bundled 'fix_issue'" in resumed.output
    assert "reloads that, not the" in resumed.output
