...
A malformed `pricing:` section is **named, never dropped in silence**. It is also what every
`policy.budget` ceiling is measured in for a provider that reports no cost, so `run`, `resume`,
`approve` and `reject` print it beside the policy warnings before the first step — see
[runs-and-resume.md](runs-and-resume.md#spending-envelopes-and-the-failure-breaker).
