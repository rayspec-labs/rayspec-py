<!-- Generated from docs/providers.md by scripts/gen_skill.py — do not edit here. -->
<!-- Canonical source: https://github.com/rayspec-labs/rayspec-py/blob/main/docs/providers.md -->
...
| `provider_options.codex` | `approval_mode` (`deny_all` default, `auto_review`), `config` (extra Codex config merged into every thread; `model`, `sandbox_mode`, `approval_policy`, `web_search` and `tools.web_search` are computed by the adapter and ignored with a warning), `ephemeral`, `usage_baseline` (usage counters **subtracted** from a resumed thread's totals — see Usage above; it sets the number every spend ceiling is measured against, so an inflated one reports no spend at all). **While any control governs the agent** the block becomes an allow-list — `config.mcp_servers`, `config.model_reasoning_summary`, `approval_mode` (`deny_all` only, once any control withholds sandbox power or network access), `ephemeral`, `usage_baseline` (every counter zero, once any spend ceiling is in force) — and every other `config` key is a load-time error; see [policy.md](https://github.com/rayspec-labs/rayspec-py/blob/main/docs/policy.md#provider_options-is-an-allow-list-while-a-control-is-in-force) |
...
priced steps (see [runs-and-resume.md](https://github.com/rayspec-labs/rayspec-py/blob/main/docs/runs-and-resume.md#failures-retries-and-timeouts)). Until a Codex model is priced, `rayspec plan`'s
...
- a `shell:`/`python:` step holds the secret and exposes a *result* the agent can reason about
  ...
  [examples/secret_via_tool](https://github.com/rayspec-labs/rayspec-py/blob/main/examples/secret_via_tool);
