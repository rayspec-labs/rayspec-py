...
| `provider_options.codex` | `approval_mode` (`deny_all` default, `auto_review`), `config` (extra Codex config merged into every thread; `model`, `sandbox_mode`, `approval_policy`, `web_search` and `tools.web_search` are computed by the adapter and ignored with a warning), `ephemeral`, `usage_baseline`. **While any control governs the agent** the block becomes an allow-list — `config.mcp_servers`, `approval_mode` (`deny_all` only under `access.max` / `network: off`), `ephemeral`, `usage_baseline` — and every other `config` key is a load-time error; see [policy.md](https://github.com/rayspec-labs/rayspec-py/blob/main/docs/policy.md#provider_options-is-an-allow-list-while-a-control-is-in-force) |
