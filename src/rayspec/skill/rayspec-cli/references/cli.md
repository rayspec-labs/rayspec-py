...
- `--commands` — Only what was executed: every command an agent ran — a Codex `command_start`, or any tool call that carries a command line, which is how the Claude adapter reports a `Bash` call — plus the rows of the `shell:`/`python:` steps the run **executed**, which are rayspec running a command itself. The dividing line is drawn **per row, not per step**: the run's own `step.started` record opens an execution and the step's next outcome record closes it, and this view is the rows inside one. So a step the run decided against before it began — `when: false`, an upstream failure, the run-level cap — ran no body and is not here, and neither is a step a resume [replayed from the reuse cache](runs-and-resume.md#resume): that row says in so many words that nothing ran. A retry *is* an execution, so a step that was retried keeps every attempt. All of them are still in the ledger proper, where being skipped or replayed is a fact about the run. Because the line is the pair of records the engine writes and not a list of statuses, it holds for whatever a step is skipped for next — and it holds within one run: on a resumed run the same step can be skipped in one attempt, executed in another and replayed in a third, and each of its rows is answered on its own. A step row names the step and its kind, not its body: the rendered body is not kept in the run directory, and `rayspec explain {run} {step}` re-renders it from the workflow.
