...
- `rayspec init [--kind code|content | --from EXAMPLE] [--force] [--no-skill] [--root DIR]`
  ...
  skill=True, doctor=True, workflow="example", stubs=True)` (additive keywords — `doctor=False`
  for a caller that has just run the check itself, `workflow=`/`stubs=` for a project whose
  workflow is not `example` or that ships no stub script for it, so `init` and `quickstart` teach
...
- `rayspec quickstart [--provider claude|codex|both|none] [--yes] [--no-interactive]
  ...
  created). `--provider` is a `LoginTarget` StrEnum, deliberately **not** doctor's repeatable
...
- **Scaffold.** `existing_project(target, project_root)` refuses to nest: a `.rayspec/` at the
  target *or* at the enclosing `find_project_root` answer means nothing is written. Otherwise
...
- **Dry run.** `dry_run_argv(workflow, *, project_root, stubs, json_)` builds the argv
  (`<wf> --dry-run [--stubs F] [--root R] --no-interactive [--json --quiet]`); `--stubs` is
  relative when the cwd already is the project root and absolute otherwise, because `--stubs`
  resolves against the cwd. `printed_command(argv)` is what is printed **and** what runs.
...
- **Isolation** (`isolation_of(declared, git, where, workflow) -> Isolation`) reads the workflow
  ...
  worktree" would be false even on a perfect machine. `next_run ∈ {worktree, none, blocked}`;
...
- **Exit codes**: `0` finished and nothing is broken (declined a login, no terminal, an existing
  project respected); `1` the environment cannot run rayspec — the dry run was performed and did
  not succeed, or it was blocked by a missing `git`; `2` usage; `130` Ctrl-C at one of
  quickstart's own questions (one line, no traceback — end of input is "not now", not 130).
- **Presentation**: report on stdout through `console()`, questions and warnings on stderr
  (`err_console()`, `typer.prompt(..., err=True)`, `typer.confirm(..., err=True)`), so stdout
  stays a clean report outside `--json` too. Glyphs are chosen once from
...
- Python surface: `QUICKSTART_KEYS`, `STEP_IDS`, `LoginTarget`, `Glyphs`, `Step`, `GitState`,
  ...
  `next_step_rows`, `payload_of`, `register`.
